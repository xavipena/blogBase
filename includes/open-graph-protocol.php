...
class OpenGraphProtocol {
	...
	/**
	 * Build Open Graph protocol text based on an array
	 *
	 * @param array $og associative array of OGP properties and values
	 * @param string $prefix optional prefix to prepend to all properties
	 */
	public static function buildTEXT( array $og, $prefix=self::PREFIX ) {
		if ( empty($og) )
			return;
		...
		foreach ( $og as $property => $content ) {
			if ( is_object( $content ) || is_array( $content ) ) {
				if ( is_object( $content ) )
					$content = $content->toArray();
				if ( empty($property) || !is_string($property) )
					$s .= static::buildTEXT( $content, $prefix );
				else
					$s .= static::buildTEXT( $content, $prefix . ':' . $property );
			} elseif ( !empty($content) ) {
				$s .= '&lt;meta ' . self::META_ATTR . '="' . $prefix;
				if ( is_string($property) && !empty($property) )
					$s .= ':' . htmlspecialchars( $property );
				$s .= '" content="' . htmlspecialchars($content) . '"&gt;<br>' . PHP_EOL;
			}
		}
		return $s;
	}
	...
	/**
	 * Output the OpenGraphProtocol object as text
	 *
	 * @return string meta elements
	 */
	public function toTEXT() {
		return rtrim( static::buildTEXT( get_object_vars($this) ), PHP_EOL );
	}
